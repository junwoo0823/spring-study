									...
									<div class="col s12 right-align">
										...
										<c:if test="${ not empty sessionScope.id }">
											...
											<c:if test="${ sessionScope.id eq board.mid }">
												<a class="btn waves-effect waves-light" href="/board/modify?num=${ board.num }&pageNum=${ pageNum }"> <i class="material-icons left">edit</i>글수정
												</a>
												<a class="btn waves-effect waves-light" onclick="remove(event)"> <i class="material-icons left">delete</i>글삭제
												</a>
											</c:if>

											<a class="btn waves-effect waves-light"
												href="/board/reply?reRef=${ board.reRef }&reLev=${ board.reLev }&reSeq=${ board.reSeq }&pageNum=${ pageNum }"> <i
												class="material-icons left">reply</i>답글
											</a>
										</c:if>

										<a class="btn waves-effect waves-light" href="/board/list?pageNum=${ pageNum }"> <i class="material-icons left">list</i>글목록
										</a>
									</div>
								</div>
							</div>
